...
";
        ...
        CodeGenFile[] generate(Dictionary<string, List<DataTuple>> contextNameToDataTuple) => contextNameToDataTuple
        ...
        string[] generateSystemListForData(string contextName, DataTuple data) => data.componentData
            .GetContextNames()
            .Where(ctxName => ctxName == contextName)
            .Select(ctxName => generateAddSystem(ctxName, data))
            .ToArray();

        string generateAddSystem(string contextName, DataTuple data) => SYSTEM_ADD_TEMPLATE
